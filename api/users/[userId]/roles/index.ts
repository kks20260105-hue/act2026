...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'DELETE') {
    ...
    if (!roleId) return res.status(400).json(errorResponse('MISSING_FIELD', 'roleId 필수'));
    ...
    await supabaseAdmin.from('tb_permission_log').insert({
      target_user_id: userId, action_type: 'REVOKE', role_id: roleId,
      role_cd_snap: (existing as any).tb_role?.role_cd ?? '',
      before_state: existing, acted_by: actor.id,
    });
