...
async function handler(req: VercelRequest, res: VercelResponse) {
  // GET /api/menus ???ÑÏ≤¥ Î©îÎâ¥ ?∏Î¶¨ (ADMIN ?¥ÏÉÅ)
  if (req.method === 'GET') {
  ...
  // POST /api/menus ??Î©îÎâ¥ ?ùÏÑ± (SUPER_ADMIN)
  if (req.method === 'POST') {
    const { menu_nm, menu_url, parent_menu_id, menu_depth, menu_order, icon_class, use_yn } = req.body;

    if (!menu_nm || !menu_url || !menu_depth) {
      return res.status(400).json(errorResponse('MISSING_FIELD', 'menu_nm, menu_url, menu_depth ?ÑÏàò'));
    }
    ...
    return res.status(201).json(successResponse(data, 'Î©îÎâ¥Í∞Ä ?ùÏÑ±?òÏóà?µÎãà??'));
  }
