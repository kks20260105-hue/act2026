...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // POST /api/roles (SUPER_ADMIN ?ÑÏö©)
  if (req.method === 'POST') {
    const { role_cd, role_nm, role_desc, role_color, sort_order } = req.body;
    if (!role_cd || !role_nm) {
      return res.status(400).json(errorResponse('MISSING_FIELD', 'role_cd, role_nm ?ÑÏàò'));
    }
    if (!/^[A-Z_]+$/.test(role_cd)) {
      return res.status(400).json(errorResponse('INVALID_FORMAT', 'role_cd???ÄÎ¨∏Ïûê?Ä _ Îß??àÏö©'));
    }
    ...
    return res.status(201).json(successResponse(data, 'Role???ùÏÑ±?òÏóà?µÎãà??'));
  }
