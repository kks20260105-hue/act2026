#!/usr/bin/env node
// tmp_revoke_flow.js - revive (if needed) then revoke mapping
...
async function main() {
  ...
  console.log('Before:', JSON.stringify(urBefore, null, 2));

  // try to ensure active mapping exists (revive if needed)
  let active = (urBefore || []).find((x) => x.role_id === role.role_id && x.use_yn === 'Y');
  if (!active) {
    const inactive = (urBefore || []).find((x) => x.role_id === role.role_id && x.use_yn === 'N');
    if (inactive) {
      console.log('Reviving inactive mapping:', inactive.user_role_id);
      const { data: reviveRes, error: reviveErr } = await admin
        .from('tb_user_role')
        .update({ use_yn: 'Y', updated_at: new Date().toISOString() })
        .eq('user_role_id', inactive.user_role_id)
        .select('*')
        .single();
      if (reviveErr) { console.error('Revive error:', reviveErr); process.exit(1); }
      console.log('Revive result:', reviveRes);
      active = reviveRes;
    } else {
      // create new mapping
      console.log('Creating new mapping (grant) for user-role');
      const { data: insertRes, error: insertErr } = await admin
        .from('tb_user_role')
        .insert([{ user_id: user.id, role_id: role.role_id, start_dt: new Date().toISOString().slice(0,10), use_yn: 'Y' }])
        .select('*')
        .single();
  ...
  // Now revoke the active mapping (delete)
  ...
  console.log('After:', JSON.stringify(urAfter, null, 2));
