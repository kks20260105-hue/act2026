...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // POST /api/menu-roles ???®Ïùº Îß§Ìïë Ï∂îÍ?
  if (req.method === 'POST') {
    const { menu_id, role_id, read_yn, write_yn } = req.body;
    if (!menu_id || !role_id) {
      return res.status(400).json(errorResponse('MISSING_FIELD', 'menu_id, role_id ?ÑÏàò'));
    }
    ...
    return res.status(201).json(successResponse(data, 'Îß§Ìïë??Ï∂îÍ??òÏóà?µÎãà??'));
  }

  // DELETE /api/menu-roles?id=xxx
  if (req.method === 'DELETE') {
    const { id } = req.query;
    if (!id) return res.status(400).json(errorResponse('MISSING_FIELD', 'id ?ÑÏàò'));
    const { error } = await supabaseAdmin.from('tb_menu_role').delete().eq('id', id as string);
    if (error) return res.status(500).json(errorResponse('DB_ERROR', error.message));
    return res.status(200).json(successResponse(null, 'Îß§Ìïë????†ú?òÏóà?µÎãà??'));
  }
